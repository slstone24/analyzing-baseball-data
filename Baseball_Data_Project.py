...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    stat_list = []
    for item in statistics:
        player_stat = formula(info, item)
        player_id = item[info['playerid']]
        tup = (player_id, player_stat)
        tuple(tup)
        stat_list.append(tup)
    stat_list.sort(key = lambda pair : pair[1] , reverse = True)
    return stat_list[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    for item in top_ids_and_stats:
        ...
        if playerid in data:
            firstname = data[playerid][info['firstname']]
            lastname = data[playerid][info['lastname']]
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    stats = read_csv_as_list_dict(info["battingfile"], info["separator"],
                                                       info["quote"])
    batting_stat = filter_by_year(stats, year, info["yearid"])
    player_list = top_player_ids(info, batting_stat, formula, numplayers)
    stat_list = lookup_player_names(info, player_list)
    return stat_list
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    aggr_stats_dict = list(aggr_stats.values())
    top_stats = top_player_ids(info, aggr_stats_dict, formula, numplayers)
